...
- [x] 5. Terraform module — `ingestion` (Step Functions + Lambda ingestion functions + EventBridge)
  - [x] 5.1 Write `terraform/modules/ingestion/iam.tf` defining `aws_iam_role.ingestion_lambda_role` with `s3:GetObject` on image buckets, `bedrock:InvokeModel` on Titan Embed and Nova Lite, `s3vectors:PutVectors`/`CreateIndex`, and `dynamodb:PutItem` on the images table
  ...
  - [x]* 5.5 Verify `terraform validate` passes for the `ingestion` module
  - _Requirements: 12.1, 12.2, 12.3, 12.4, 13.1_

- [x] 6. Terraform module — `api` (API Gateway + `api_handler` Lambda + IAM + usage plan)
  ...
  - [x]* 6.5 Verify `terraform validate` passes for the `api` module
  - _Requirements: 2.1, 2.2, 2.3, 3.1, 3.2, 9.1, 9.2, 10.4, 13.1_

- [x] 7. Terraform root modules — wire all modules into environments
  ...
  - [x] 7.2 Replicate the same root module structure for `staging` and `prod` environments
  - [x]* 7.3 Run `terraform validate` and `terraform plan -detailed-exitcode` (with a mocked/stub backend) for each environment; confirm plan exit code 2 (changes pending, no errors)
    - **Done (2026-09-11):** `terraform fmt -check` and `terraform validate` pass for all three env roots (dev, staging, prod) — validated via `terraform init -backend=false` + `terraform validate`. `terraform plan -detailed-exitcode` was **not** run here: it requires live AWS credentials and the real S3 remote backend (there is no mock/stub backend that resolves the account/region data sources and module resources for a plan). Run the plan step in CI or against a dev account per the terraform-conventions CI/CD section.
  - _Requirements: 13.1, 13.2, 13.3, 13.4, 13.5_

- [x] 8. Shared API library — exceptions, error handler, and response helpers
  ...
  - [x] 8.3 Create `src/api_handler/middleware.py` with a Powertools middleware (or exception handler decorator) that catches each exception class and calls `error_response` with the mapped status and token; the catch-all for bare `Exception` logs the full traceback to CloudWatch and returns a 500 with `server.error`
...
- [x] 9. Shared API library — query parameter parsing and validation
...
- [x] 10. Shared API library — router setup and method/version enforcement
  - [x] 10.1 Create `src/api_handler/app.py` initialising `APIGatewayRestResolver` from AWS Lambda Powertools; register the router; add a before-request hook that raises `MethodNotAllowedError` for any non-GET method reaching a registered path; add a path-prefix guard that returns 404 for paths not starting with `/v1/`
...
- [x] 12. Route handler — `GET /v1/collections`
...
- [x] 13. Route handler — `GET /v1/collections/{collection_name}`
...
- [x] 14. Route handler — `GET /v1/collections/{collection_name}/images`
...
- [x] 15. Route handler — `GET /v1/collections/{collection_name}/images/{image_key}`
...
- [x] 16. Checkpoint — all API route handlers complete
...
- [x] 17. Ingestion Lambda — `ingestion_embed`
...
- [x] 18. Ingestion Lambda — `ingestion_describe`
...
- [x] 19. Ingestion Lambda — `ingestion_store`
...
- [x] 22. Wire routes into Powertools app and finalize Lambda package
...
- [x] 23. Auth property tests and sort property tests
  - [x]* 23.1 Write property tests in `tests/property/test_auth_properties.py` — Property 3 (generate valid/invalid JWT structures against a mock JWKS; assert allow/reject behavior)
    - **Not unit-testable — documented skip (2026-09-11):** JWT validation is performed entirely by the AWS-managed API Gateway Cognito authorizer (`COGNITO_USER_POOLS`, see `terraform/modules/api/apigw.tf`). There is **no JWT/JWKS code in `src/`** to exercise (confirmed: the only auth reference is the `COGNITO_USER_POOL_ID` config passthrough). A Hypothesis+moto test would have to fabricate its own verification logic, testing invented code rather than the system. `tests/property/test_auth_properties.py` is therefore a documented module-level `pytest.skip`. Property 3 is genuinely verified against the deployed API Gateway by the integration test in task 25.1.
  - [x]* 23.2 Write property tests in `tests/property/test_sort_properties.py` — Property 7 (generate arbitrary strings as `sort` parameter; assert non-valid sort fields return 400 for both collections and images endpoints)
