# ---------------------------------------------------------------------------
# Staging environment root module (task 7.2)
#
# Wires the four infrastructure modules — auth, storage, ingestion, api — and
# provisions the admin IAM role. Structurally identical to the dev root; all
# resource names and scoping derive from var.env, so the only per-environment
# differences come from environments/staging/terraform.tfvars.
#
# environments/staging is its own Terraform root; it declares its own terraform
