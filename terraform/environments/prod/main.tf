# ---------------------------------------------------------------------------
# Prod environment root module (task 7.2)
#
# Wires the four infrastructure modules — auth, storage, ingestion, api — and
# provisions the admin IAM role. Structurally identical to the dev/staging
# roots; all resource names and scoping derive from var.env, so the only
# per-environment differences come from environments/prod/terraform.tfvars.
#
# environments/prod is its own Terraform root; it declares its own terraform
# block, provider, and S3 backend. Backend values are supplied at init time via
# -backend-config=backend.config.
#
# Note: apply to prod requires manual approval (see terraform-conventions.md
# CI/CD section — prod is never auto-applied on merge).
# ---------------------------------------------------------------------------
